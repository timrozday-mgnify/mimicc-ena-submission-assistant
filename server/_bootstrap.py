...
This app reuses code from two sibling repositories:
  ...
  * ``ena-submission-dataharmonizer``  -> ``ena_common``, ``submit_sample``,
    ``submit_study``, ``prepare_dh_output`` and the ``linkml_lib`` package.
...
for _p in (_ENA_API_ROOT, SCRIPTS_DIR):
...
def schema_path() -> Path:
    found = _first_existing(
        ...
        _VENDOR / "schemas" / "mimicc_sample_experiment.yaml",
        _SIBLINGS / "ena-submission-dataharmonizer" / "schemas" / "mimicc_sample_experiment.yaml",
    )
    if found is None:
        raise RuntimeError(
            "Could not locate mimicc_sample_experiment.yaml. Run scripts/vendor.sh or set ENA_DH_SCHEMA."
        )
    return found
